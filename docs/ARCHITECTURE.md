...
2. **Images via CI.** Code changes trigger an image build pushed to a registry, and the Deployment is updated by tag. More moving parts, but rollback is native (`kubectl rollout undo`) and the artifact is reproducible.
   ...
   **[Decided 2026-07-28]** The implementation is **Zot** (`registry` namespace, 50Gi local-path PVC). Chosen over Harbor, which drags Postgres and Redis for RBAC and scanning features a single user does not need, and over Distribution, whose garbage collection requires stopping the registry. Zot runs online GC on a schedule, deduplicates layers, and needs no database.

   Operational shape, all verified end to end on 2026-07-28:
   - Canonical name is `zot.registry.svc.cluster.local:5000`. In-cluster clients resolve it through CoreDNS; the node's containerd cannot resolve `.svc.cluster.local`, so `/etc/rancher/k3s/registries.yaml` mirrors that same name to `http://127.0.0.1:30500` (the NodePort) with credentials. One image reference works from both sides.
   - It speaks **plain HTTP**, so OCI clients need an insecure/`--insecure` flag. This is deliberate: the only paths that reach it are the pod network and the tailnet, and the tailnet is already WireGuard-encrypted. Nothing travels unprotected. The alternative (internal CA plus trust distribution to every client) buys no real confidentiality here and adds a certificate to maintain.
   - Auth is htpasswd (bcrypt), user `gaia`, admin policy; anonymous access denied. The credential lives in the `zot-credentials` Secret in the `registry` namespace and nowhere else.
   - `readTimeout`/`writeTimeout` are raised to 3600s. The chart defaults to 60s, which is precisely the limit that breaks pushes of multi-gigabyte layers.
   - The tailnet is allowed into the node (`-i tailscale0 -j ACCEPT`) so the NodePort is reachable over the VPN; the public internet still hits the REJECT rule.
...
- **Private network (VPN).** **[Reopened and decided 2026-07-28]** The earlier decision deferred the VPN out of v0 entirely; Paulo reversed it. Tailscale is in, and its job is broader than the original framing: it is the access path to any port or DNS name that should not be on the public internet, the CLI agent talking directly to memory being one case among several. It is also expected to pair with the deploy-control tool planned for much later. It is a split-tunnel style VPN and should coexist with the work OpenVPN (which is not oppressive); this coexistence is still unverified in practice.
  ...
  **How a service becomes private, in practice. [Decided 2026-07-28, working.]** The Tailscale Kubernetes operator runs in the `tailscale` namespace. Anything with a UI or a DNS name that should not be public gets an `Ingress` with `ingressClassName: tailscale`; the operator creates a dedicated tailnet device for it and terminates TLS with a real Let's Encrypt certificate for `<name>.bilby-tench.ts.net`. No wildcard is involved and none is possible: Tailscale resolves wildcard DNS but does not issue wildcard certificates. One device and one certificate per service is the model, and it is better than a wildcard because ACLs can then address each service individually.

  Use `Ingress`, not a `LoadBalancer` Service, whenever a browser is involved. A `Service` with `loadBalancerClass: tailscale` is layer 4 only: it forwards TCP without terminating TLS, so an HTTP backend published on port 443 answers plaintext to a client expecting TLS.

  **Naming consequence worth knowing:** pods resolve through CoreDNS and cannot resolve `.ts.net` names, so in-cluster clients must use the internal Service name while external clients on the tailnet use the `.ts.net` name. Two names for one service is expected, not a misconfiguration.

  **The `tagOwners` trap, cost an hour on 2026-07-28.** A tagged OAuth client acts *as a tag*, never as the human who created it. With `"tag:k8s-operator": ["autogroup:admin"]` the operator cannot register itself, because it is not an admin: creating its own auth key fails with `requested tags are invalid or not permitted (400)`. The tag must own itself: `"tag:k8s-operator": ["autogroup:admin", "tag:k8s-operator"]`. The giveaway is that a *different* tag owned by `tag:k8s-operator` is accepted while its own tag is refused, which proves the policy file is saved and valid.

  **A custom domain resolving only inside the tailnet (`something.paulocarmino.com`, private) is a known path, deliberately not built.** It needs a second `EnvoyProxy` with `loadBalancerClass: tailscale` plus its own `GatewayClass`, ExternalDNS watching Gateways, an authoritative internal DNS resolver, and Split DNS configured in the Tailscale admin console. Four components to gain nothing but a prettier name over what `.ts.net` already delivers. Build it when a concrete case demands that exact hostname, per the CONVENTIONS rule on patterns earning their place.
