...
- 2026-07-28, anything with a UI or a private DNS name is published with an `Ingress` of `ingressClassName: tailscale` and lives at `<name>.bilby-tench.ts.net`, public hostnames on `paulocarmino.com` are the exception requested case by case (Paulo's rule); one tailnet device and one certificate per service, since Tailscale resolves wildcard DNS but issues no wildcard certificates.
- 2026-07-28, use a tailscale `Ingress` and never a `loadBalancerClass: tailscale` Service when a browser is involved, the Service is layer 4 and forwards TCP without terminating TLS, so an HTTP backend on port 443 answers plaintext to a client expecting TLS.
- 2026-07-28, a tag in `tagOwners` must own itself (`"tag:k8s-operator": ["autogroup:admin", "tag:k8s-operator"]`), a tagged OAuth client acts as a tag and not as its human creator, so without this the operator cannot mint its own auth key and dies with `requested tags are invalid or not permitted (400)`.
- 2026-07-28, in-cluster clients address services by their internal Service name and tailnet clients by the `.ts.net` name, pods resolve through CoreDNS which cannot resolve `.ts.net`, so two names for one service is expected rather than a mistake.
- 2026-07-28, a private custom domain inside the tailnet is a known but unbuilt path, it costs a second EnvoyProxy and GatewayClass plus ExternalDNS, an authoritative internal resolver and Split DNS, to gain only a nicer name than `.ts.net` already provides.
- 2026-07-28, local development database is Postgres 18 in docker-compose (`pgvector/pgvector:pg18`), the Postgres trigger fired because kanban/tasks/projects attached to conversations are the same relational aggregate as conversations themselves, so conversations must not be born in SQLite only to be migrated later with cross-database foreign keys.
- 2026-07-28, PostgreSQL major is pinned to 18 across dev and production, CloudNativePG 1.30.x supports 14-18 so 18 is the newest supported; INFRA TASK: the k3s CloudNativePG cluster must be created as major 18 to match, and `pgvector` must be available.
- 2026-07-28, Docker is fine for local development infrastructure (the database), the earlier "no Docker" scope note was about not containerising GAIA herself in local v0, and the "no Docker on the host" decision is specifically about the k3s host where the daemon breaks pod networking.
- 2026-07-28, the Postgres 18 container image wants its volume mounted at `/var/lib/postgresql` and NOT at `/var/lib/postgresql/data` as in 17 and earlier, the old path crashloops with an "unused mount/volume" error; 18 keeps the cluster in a version-specific subdirectory so `pg_upgrade --link` does not cross a mount boundary.
- 2026-07-28, OPEN QUESTION, do not treat as decided: `packages/mnemosyne` was written from scratch, but Mnemosyne is an existing open-source project (github.com/mnemosyne-oss/mnemosyne, Python, SQLite, hybrid vector+FTS5 search, BEAM working/episodic/knowledge tiers, MCP over SSE, automatic consolidation) which is almost certainly what the architecture document meant; the memory strategy is being re-evaluated from scratch in a dedicated session before any more is built on our own implementation.
