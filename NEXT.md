...
- **No backups, and this is now a deliberate deferral.** Paulo reopened the
  decision on 2026-07-28: backups are no longer a prerequisite, and Oracle Object
  Storage is no longer settled as the destination. See ARCHITECTURE, Backups. Two
  things follow. GAIA's arms stay pointed at targets whose loss would be an
  annoyance rather than a real loss until backups exist. And every new service
  keeps its durable state in one known directory, so that whatever tool and
  destination eventually win, adopting them is a small job.
...
> consolidation.
>
> Read the loose ends in NEXT.md before you start. Backups are deliberately
> deferred and are not a gate; do not re-litigate that. What it costs you is a
> constraint: the arms only ever touch a scratch repo until backups exist, and any
> durable state you add lives in one known directory per service. There is also no
> tested rollback path, which ARCHITECTURE still requires before GAIA modifies
> herself, so her own repo is out of bounds as an arm target for now. Also worth
> knowing: steering is turn-granular, and `agent.abort()` is not wired to any
> endpoint.
>
