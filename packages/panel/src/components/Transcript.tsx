...
const Prose = ({ text }: { text: string }) => (
  <div
    className="text-term-fg max-w-none text-[13px] leading-[1.65]
      ...
      [&_code]:bg-term-panel [&_code]:px-1 [&_code]:py-0.5 [&_code]:text-term-tool [&_code]:text-[12px]
      [&_em]:text-term-dim [&_em]:not-italic
      [&_h1]:mt-4 [&_h1]:mb-2 [&_h1]:text-[13px] [&_h1]:tracking-widest [&_h1]:uppercase [&_h1]:text-term-dim
      [&_h2]:mt-4 [&_h2]:mb-2 [&_h2]:text-[13px] [&_h2]:tracking-widest [&_h2]:uppercase [&_h2]:text-term-dim
      [&_h3]:mt-3 [&_h3]:mb-1 [&_h3]:text-[13px] [&_h3]:text-term-fg
  ...
  return (
    <div className="border-term-line/70 mt-2 border-l-2 pl-3">
      <button
        ...
        className="text-term-tool hover:text-term-fg flex items-baseline gap-2 text-[11px]"
      >
        <span>{open ? "▾" : "▸"}</span>
      ...
      {open ? (
        <div className="text-term-dim mt-1.5 whitespace-pre-wrap text-[11px] leading-[1.6]">
          {body}
        </div>
      ) : null}
    </div>
...
const ToolLine = ({ entry }: { entry: Extract<Entry, { kind: "tool" }> }) => {
  ...
  return (
    <div className="border-term-line/70 my-2 flex flex-wrap items-baseline gap-x-2 border-l-2 pl-3 text-[11px]">
      {mark}
...
export const Transcript = ({ entries, running }: { entries: Entry[]; running: boolean }) => {
  ...
  if (entries.length === 0) {
    return (
      <div className="text-term-dim px-5 py-10 text-xs">
        ...
        if (entry.kind === "user") {
          return (
            <div key={entry.id} className="mb-5">
              ...
              />
              <div className="text-term-fg whitespace-pre-wrap text-[13px] leading-[1.65]">
                {entry.text}
              </div>
